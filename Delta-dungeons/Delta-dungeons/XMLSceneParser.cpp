...
std::vector<std::shared_ptr<Tile>> XMLSceneParser::loadScene(const std::string& path, const cbCollision f, void* p)
{
	...
	for (std::shared_ptr<ParserData> tile : data)
	{
		...
		if (tile.get()->tileId[1]) {
			...
			tileMap.emplace_back(std::make_shared<Tile>(std::stoi(tile.get()->x), std::stoi(tile.get()->y), first, second, tile.get()->isCollider, f, p));
		}
		else
		{
			tileMap.emplace_back(std::make_shared<Tile>(std::stoi(tile.get()->x), std::stoi(tile.get()->y), first, tile.get()->isCollider, f, p));
		}
	}
	return tileMap;
}
