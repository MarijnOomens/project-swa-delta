...
std::shared_ptr<Pokemon> GameObjectBuilder::getPokemon(int x, int y, const std::string& name, cbCollision collisionCb, cbCameraRange cameraCb, void* p, int attackTime)
{
	std::shared_ptr<Pokemon> pokemon = std::make_shared<Pokemon>(x, y, name, collisionCb, cameraCb, p, attackTime);
	pokemon->setParent();
