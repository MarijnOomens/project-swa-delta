...
void PuzzleFactory::createPuzzleOne(std::shared_ptr<ParserData> parsedData, std::string levelName, cbInteract interactCb, cbCollision collisionCb, void* p)
{
	...
	{
		puzzleObjects.insert(std::pair<std::string, std::shared_ptr<IInteractiveObject>>("puzzle1",
			builder->getPuzzle(std::stoi(parsedData->x), std::stoi(parsedData->y), "boulder_game_over", levelName, collisionCb, interactCb, p, std::stoi(parsedData->tileId))));
	}
	else if (std::stoi(parsedData->tileId) == 2)
	{
		puzzleObjects.insert(std::pair<std::string, std::shared_ptr<IInteractiveObject>>("puzzle1",
			builder->getPuzzle(std::stoi(parsedData->x), std::stoi(parsedData->y), "reset", levelName, collisionCb, interactCb, p, std::stoi(parsedData->tileId))));
	}
}
